...
> Coinbase Wallet SDK SDK — A lightweight, zero-dependency utility layer for async state management in modern JavaScript applications.
...
```javascript
const { useAsyncState } = require('Coinbase-Wallet-SDK');
...
```javascript
const { init } = require('Coinbase-Wallet-SDK');
...
MIT © Copyright (c) 2026 Coinbase Wallet SDK, Inc.
